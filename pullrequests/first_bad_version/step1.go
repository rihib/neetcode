...
right = midと更新される以上、rightがleftを飛び越えることはなくtrueの範囲にとどまり続け、left == rightになるまで探索は終了しないので、半閉区画の場合はleftの代わりにrightを返しても問題ない。

閉区画の場合、falseとtrueの境界にあるとき、rightはmid-1によりfalseの方に移動するのに対し、leftはtrueの方に移動するため、right < leftになった瞬間にleftはFirst bad versionであることがわかる。right <= leftとしてしまうと、trueの範囲内でleft == rightになる場合は問題ないが、falseの範囲内でleft == rightとなる場合は探索結果になってしまう。
right < leftになった瞬間に探索が終了するわけだが、この場合、leftはtrue、rightはfalseになっているため、閉区画の場合は必ずleftを返す必要がある。
*/
